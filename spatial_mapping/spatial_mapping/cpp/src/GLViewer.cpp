...
void GLViewer::init(int argc, char **argv, sl::Mat &image, // 用于存储实时获取的相机图像数据
                    sl::Mat &pointcloud,                   // 用于存储实时获取的点云数据
                    CUstream stream)                       // CUDA 流，用于在 GPU 上执行并行计算时优化性能，通过 zed.getCUDAStream() 获取
{
